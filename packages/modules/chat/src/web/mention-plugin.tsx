...
// ── Nó de menção ──────────────────────────────────────────────────────────────
// Token atômico que mostra "@Nome" no editor e carrega o userId. Serializa para
// o formato `@[Nome](user:ID)` no markdown (ver MENTION_TRANSFORMER) — assim a
// identidade do membro sobrevive ao wire/banco e é re-hidratada na edição.
...
export class MentionNode extends TextNode {
  ...
  /** Nome sem o "@" — usado na exportação para markdown. */
  getName(): string {
  ...
  // Token indivisível: não dá para digitar "dentro" da menção.
...
/**
 * Autocomplete inline estilo Slack/Discord: "@an" lista os membros do projeto;
 * selecionar insere um MentionNode. `menuOpenRef` avisa o EnterToSendPlugin para
 * não enviar enquanto o menu está aberto.
 */
export function MentionTypeaheadPlugin({
  ...
  const { members, currentUserId } = useChat();
