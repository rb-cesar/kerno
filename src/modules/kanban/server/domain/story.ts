import { prisma } from "@/core/db";
import { createEvent, eventBus } from "@/core/events";
import type { Priority, StatusCategory } from "../../types";

export class StoryDomain {
  /** Publica o resync genérico do Kanban p/ os outros clientes do workspace. */
  private notifyChanged(boardId: string, workspaceId: string, actorId: string) {
    eventBus.publish(
      createEvent("kanban:changed", workspaceId, { boardId, cardId: null }, actorId),
    );
  }

  /** Cria uma User Story no board, numerada por board (em transação) e ao final da ordem. */
  async createStory(boardId: string, title: string, actorId: string) {
    const board = await prisma.board.findUniqueOrThrow({
      where: { id: boardId },
      select: { workspaceId: true },
    });
    const story = await prisma.$transaction(async (tx) => {
      const agg = await tx.story.aggregate({
        where: { boardId },
        _max: { number: true, order: true },
      });
      const number = (agg._max.number ?? 0) + 1;
      const order = (agg._max.order ?? -1) + 1;
      return tx.story.create({ data: { boardId, title, number, order } });
    });
    this.notifyChanged(boardId, board.workspaceId, actorId);
    return story;
  }

  async updateStory(
    input: {
      storyId: string;
      title: string;
      description: string | null;
      status: StatusCategory;
      assignedTo: string | null;
      dueDate: Date | null;
      priority: Priority;
      color: string | null;
    },
    actorId: string,
  ) {
    const story = await prisma.story.update({
      where: { id: input.storyId },
      data: {
        title: input.title,
        description: input.description,
        status: input.status,
        assignedTo: input.assignedTo,
        dueDate: input.dueDate,
        priority: input.priority,
        color: input.color,
      },
      select: { id: true, boardId: true, board: { select: { workspaceId: true } } },
    });
    this.notifyChanged(story.boardId, story.board.workspaceId, actorId);
  }

  async deleteStory(storyId: string, actorId: string): Promise<void> {
    const story = await prisma.story.delete({
      where: { id: storyId },
      select: { boardId: true, board: { select: { workspaceId: true } } },
    });
    this.notifyChanged(story.boardId, story.board.workspaceId, actorId);
  }

  /** Workspace dono de uma story — usado pela camada de permissão. */
  async workspaceIdOfStory(storyId: string): Promise<string | null> {
    const story = await prisma.story.findUnique({
      where: { id: storyId },
      select: { board: { select: { workspaceId: true } } },
    });
    return story?.board.workspaceId ?? null;
  }
}

export function createStoryDomain(): StoryDomain {
  return new StoryDomain();
}

/** Instância pronta pra uso — stateless, sem motivo pra cada consumidor criar a sua. */
export const story = createStoryDomain();
