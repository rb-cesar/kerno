...
export class MetricsDomain {
  ...
  private weekStartDate(d: Date): Date {
    const x = new Date(d);
    x.setHours(0, 0, 0, 0);
  ...
  private lastNWeeks(n: number): string[] {
    const cur = this.weekStartDate(new Date());
    const out: string[] = [];
   ...
   */
  async getBoardMetrics(boardId: string): Promise<BoardMetricsDTO> {
    ...
    for (const e of events) {
      if (e.category === "STARTED" && !firstStarted.has(e.cardId)) {
        firstStarted.set(e.cardId, e.at);
      }
    ...
    for (const [cardId, completedAt] of firstCompleted) {
      ...
      const wk = this.isoDay(this.weekStartDate(completedAt));
    ...
    return {
      ...
      avgCycleTimeDays: cycleN > 0 ? this.round1(cycleSum / cycleN) : null,
      avgLeadTimeDays: leadN > 0 ? this.round1(leadSum / leadN) : null,
      throughput: this.lastNWeeks(8).map((weekStart) => ({
        weekStart,
