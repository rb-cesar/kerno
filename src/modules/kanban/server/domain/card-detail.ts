...
export class CardDetailDomain {
  /** Cria uma sub-tarefa: um card filho no mesmo estado (coluna) do pai. */
  async createSubtask(parentId: string, title: string, actorId: string) {
    const parent = await prisma.card.findUniqueOrThrow({
      where: { id: parentId },
      select: { columnId: true },
    });
    return card.createCard(parent.columnId, title, actorId, parentId);
  }
  ...
  async cardIdOfComment(commentId: string): Promise<string | null> {
    const comment = await prisma.cardComment.findUnique({
      where: { id: commentId },
      select: { cardId: true },
    });
   ...
   */
  async getCardDetail(cardId: string, viewerId: string): Promise<CardDetailDTO> {
    const cardRow = await prisma.card.findUniqueOrThrow({
      where: { id: cardId },
      select: { boardId: true },
    });
...
export function createCardDetailDomain(): CardDetailDomain {
  return new CardDetailDomain();
}

/** Instância pronta pra uso — stateless, sem motivo pra cada consumidor criar a sua. */
export const cardDetail = createCardDetailDomain();
