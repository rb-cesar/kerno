...
import { kanbanGuards } from "./guards";
...
export class KanbanService {
  ...
  async boardForWorkspace(userId: string, workspaceId: string): Promise<BoardData> {
    await kanbanGuards.assertMember(userId, workspaceId);
  ...
  async snapshot(userId: string, boardId: string): Promise<BoardData> {
    await kanbanGuards.guardBoard(userId, boardId);
  ...
  async cardDetail(userId: string, cardId: string): Promise<CardDetailDTO> {
    await kanbanGuards.guardCard(userId, cardId);
  ...
  async runCommand(userId: string, command: KanbanCommand): Promise<KanbanMutationResult> {
    try {
      ...
      switch (command.type) {
        case "createBoard": {
          await kanbanGuards.assertMember(userId, command.workspaceId, "MEMBER");
        ...
        case "renameBoard": {
          await kanbanGuards.guardBoard(userId, command.boardId, "MEMBER");
        ...
        case "renameColumn": {
          await kanbanGuards.guardColumn(userId, command.columnId, "MEMBER");
        ...
        case "updateColumn": {
          ...
          const name = command.name.trim();
          if (!name) return { ok: false, error: "Nome inválido" };
          const wipLimit = command.wipLimit != null && command.wipLimit > 0 ? command.wipLimit : null;
        ...
        case "updateCard": {
          await kanbanGuards.guardCard(userId, command.cardId, "MEMBER");
        ...
        case "createSubtask": {
          await kanbanGuards.guardCard(userId, command.parentId, "MEMBER");
        ...
        case "createChecklist": {
          await kanbanGuards.guardCard(userId, command.cardId, "MEMBER");
          await domain.checklist.createChecklist(command.cardId, command.title?.trim() || null, userId);
          break;
        }
        case "renameChecklist": {
          await kanbanGuards.guardChecklist(userId, command.checklistId, "MEMBER");
          await domain.checklist.renameChecklist(command.checklistId, command.title?.trim() || null, userId);
          break;
        }
        ...
        case "toggleChecklistItem": {
          await kanbanGuards.guardChecklistItem(userId, command.itemId, "MEMBER");
        ...
        case "deleteComment": {
          await kanbanGuards.guardComment(userId, command.commentId, "MEMBER");
        ...
        case "deleteLabel": {
          await kanbanGuards.guardLabel(userId, command.labelId, "MEMBER");
        ...
        case "deleteCycle": {
          await kanbanGuards.guardCycle(userId, command.cycleId, "MEMBER");
        ...
        case "updateStory": {
          await kanbanGuards.guardStory(userId, command.storyId, "MEMBER");
          const title = command.title.trim();
          if (!title) return { ok: false, error: "Título vazio" };
