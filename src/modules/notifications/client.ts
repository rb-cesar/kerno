...
export const notificationsClient = {
  fetch: (beforeId?: string): Promise<NotificationsData> =>
    request<NotificationsData>(
      beforeId ? `/notifications?before=${encodeURIComponent(beforeId)}` : "/notifications",
    ).catch(() => ({ items: [], unreadCount: 0, hasMore: false })),
