...
import { DEFAULT_THEME, THEME_GROUPS, type ThemeMeta } from "@/components/theme/themes";
import { cn, Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui";
          ...
          {THEME_GROUPS.map(({ group, themes }) => (
            <section key={group}>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {themes.map((t) => (
                  <ThemeCard key={t.id} theme={t} selected={current === t.id} onSelect={() => setTheme(t.id)} />
                ))}
              </div>
            </section>
          ))}
...
function ThemeCard({ theme, selected, onSelect }: { theme: ThemeMeta; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? "border-primary ring-2 ring-primary" : "border-border hover:border-muted-foreground/50",
      )}
    >
      ...
      <div className="h-20 w-full" style={{ backgroundColor: bg }}>
        {/* header */}
        <div className="flex h-3 w-full items-center gap-1 px-1.5" style={{ backgroundColor: header }}>
          <span className="h-1 w-6 rounded-full" style={{ backgroundColor: fg, opacity: 0.5 }} />
        </div>
        <div className="flex h-[68px]">
          {/* barra de hubs */}
          <div className="flex w-4 flex-col items-center gap-1 py-1.5" style={{ backgroundColor: sidebar }}>
